...
(function () {
  ...
  /* ---------------- data model ---------------- */
  const TRACKS = [window.TRACKS.mlsd, window.TRACKS.mlcase].filter(Boolean);
  const QUIZZES = window.QUIZZES || {};
  const ACTIVE_QUIZ_PREFIXES = ["mlsd-", "mlcase-"];
  ...
  const Practice = window.SynapsePractice || {};
  ...
  const LEARNING_PATHS = [
    {
      id: "mlsd-foundations",
      title: "MLSD Foundations",
      label: "Foundations",
      color: "#a78bfa",
      desc: "The interview shape, the six-phase framework, and how a business goal becomes an ML objective you can measure.",
      routes: [
        "#/mlsd/framework/what-is-mlsd",
        "#/mlsd/framework/phase-plan",
        "#/mlsd/framework/problem-framing",
        "#/mlsd/framework/high-level-design",
        "#/mlsd/framework/assessment",
        "#/mlsd/framework/level-expectations"
      ]
    },
    {
      id: "mlsd-modelling",
      title: "Modelling Depth",
      label: "Modelling",
      color: "#38bdf8",
      desc: "Embeddings, features, training pipelines, generalization, and the metrics that decide whether any of it worked.",
      routes: [
        "#/mlsd/concepts/embeddings",
        "#/mlsd/concepts/ann-serving",
        "#/mlsd/concepts/feature-engineering",
        "#/mlsd/concepts/feature-pitfalls",
        "#/mlsd/concepts/feature-stores",
        "#/mlsd/concepts/training-pipelines",
        "#/mlsd/concepts/generalization",
        "#/mlsd/concepts/evaluation",
        "#/mlsd/concepts/ab-testing"
      ]
    },
    {
      id: "mlsd-serving",
      ...
      label: "Serving",
      color: "#34d399",
      desc: "Two-stage retrieval, inference architecture, drift monitoring, and closing the feedback loop without poisoning it.",
      routes: [
        "#/mlsd/serving/retrieval-ranking",
        "#/mlsd/serving/inference-architecture",
        "#/mlsd/serving/drift-monitoring",
        "#/mlsd/serving/feedback-loops"
      ]
    },
    {
      id: "mlsd-drills",
      title: "Breakdown Drills",
      label: "Breakdowns",
      color: "#f472b6",
      desc: "Seven full walkthroughs, each graded Naive / Solid / Standout with an explicit mid, senior and staff bar.",
      routes: [
        "#/mlcase/recsys/video-recommendations",
        "#/mlcase/recsys/feed-ranking",
        "#/mlcase/recsys/ad-click-prediction",
        "#/mlcase/trust/harmful-content",
        "#/mlcase/trust/bot-detection",
        "#/mlcase/retrieval/search-ranking",
        "#/mlcase/retrieval/rag-assistant"
      ]
    }
  ];

  /* ---------------- progress (localStorage) ---------------- */
  const PKEY = "sy_progress_v1";
  let done = new Set();
  let activeQuiz = null;
  ...
  function renderHome() {
    document.title = "Synapse · The ML System Design Atlas";
    const qTotal = activeQuizKeys().reduce((a, k) => a + QUIZZES[k].questions.length, 0);
    const feat = [
      ["bolt", "Interactive labs", "Tune a precision/recall guardrail, walk a two-tower retrieval index, watch feature drift bend a live metric, size an inference budget, and grade your own objective against the Naive/Solid/Standout ladder."],
    ...
    main.innerHTML =
      '<section class="hero">' +
        '<span class="hero-tag reveal reveal-1"><span class="pulse"></span>Interactive ML system-design atlas</span>' +
        '<h1 class="reveal reveal-2">Design ML systems<br>that <span class="grad">learn</span> &amp; ship<br>models that <span class="grad">hold up</span>.</h1>' +
        '<p class="lede reveal reveal-3">Work the machine-learning design interview end to end \u2014 turning a fuzzy business goal into a measurable ML objective, choosing features and models you can defend, and serving predictions inside a real latency budget. Then walk seven full problem breakdowns, graded Naive \u2192 Solid \u2192 Standout.</p>' +
        '<div class="hero-cta reveal reveal-4">' +
          (resumeF
            ? '<a class="btn btn-primary" href="' + resumeF.route + '">Resume \u00b7 ' + escapeHtml(resumeF.lesson.title) + ARR + "</a>" +
              '<a class="btn btn-ghost" href="#/mlsd/framework/what-is-mlsd">Start the framework' + ARR + "</a>"
            : '<a class="btn btn-primary" href="#/mlsd/framework/what-is-mlsd">Start the framework' + ARR + "</a>" +
              '<a class="btn btn-ghost" href="#/mlcase/recsys/video-recommendations">Jump to a breakdown' + ARR + "</a>") +
      ...
      '<a class="practice-banner" href="#/practice">' +
        ...
        '<div class="pb-text"><h3>Test yourself in Practice mode</h3><p>A shuffled mix of every MLSD and CASES checkpoint quiz \u2014 ' + qTotal + ' questions for spaced-repetition review.</p></div>' +
  ...
  function renderPaths() {
    document.title = "Learning paths · Synapse";
    ...
    main.innerHTML =
      '<article class="lesson paths-page" style="--accent:var(--cyan)">' +
        ...
        '<header class="lesson-head reveal in">' +
          ...
          '<p class="summary">Opinionated routes through existing Synapse lessons. Use them when you want a focused study sequence instead of browsing the full atlas.</p>' +
        "</header>" +
  ...
  function practicePageShell(kind, body, anchor) {
    ...
    document.title = meta.title + " · Synapse";
  ...
  function renderPractice(opts) {
    ...
    document.title = "Practice mode · Synapse";
    ...
    if (mode === "weak" && !picked.length) {
      ...
      mountQuiz($("#practiceSlot"), {
        ...
        sub: count + (mode === "weak" ? " missed question" + (count === 1 ? "" : "s") + " to re-master" : " questions, shuffled across MLSD & CASES"),
        questions: picked
      });
    }
    main.scrollTop = 0;
  ...
  function renderReview() {
    document.title = "Study list · Synapse";
  ...
  function renderLesson(ctx) {
    ...
    document.title = lesson.title + " · Synapse";
    saveLast(ctx.key);
  ...
  function pathToMd(path) {
    const lessons = pathLessons(path);
    let out = "# " + path.title + " \u2014 Synapse learning path\n\n";
    out += "> " + path.desc + "\n\n";
    lessons.forEach((f, i) => {
  ...
  function renderExam() {
    document.title = "Exam mode \u00b7 Synapse";
  ...
  function renderFlashcards() {
    document.title = "Flashcards \u00b7 Synapse";
  ...
  function renderPrint(trackId) {
    ...
    document.title = track.name + " cheat sheet \u00b7 Synapse";
  ...
  function exportData() {
    ...
    const payload = { app: "synapse", version: 1, exportedAt: new Date().toISOString(), data };
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = el("a", { href: url, download: "synapse-progress-" + new Date().toISOString().slice(0, 10) + ".json" });
  ...
  function importData(file) {
    ...
    reader.onload = () => {
      ...
      if (!parsed || parsed.app !== "synapse" || !parsed.data) { toast("Import failed \u2014 not a Synapse backup"); return; }
