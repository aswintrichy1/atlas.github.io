...
(function () {
  ...
  function renderHome() {
    ...
    main.innerHTML =
        ...
        '<div class="hero-cta reveal reveal-4">' +
          (resumeF
            ...
            : '<a class="btn btn-primary" href="#/dsa/foundations/arrays">Start with DSA' + ARR + "</a>" +
              '<a class="btn btn-ghost" href="#/patterns/arrays/prefix-sum">17 pattern lessons' + ARR + "</a>") +
      ...
      '<a class="path-banner" href="#/paths">' +
        ...
        '<div class="pb-text"><h3>Follow a guided learning path</h3><p>Pick DSA Foundations, Algorithm Core, Graphs, Recursion & DP, Pattern Mastery, or Interview Sprint and copy the path as Markdown study notes.</p></div>' +
  ...
  function mountQuiz(slot, quiz) {
    ...
    function choose(oi, opts, q, explain, nextBtn) {
      ...
      btns.forEach((b, bi) => {
        b.disabled = true;
        const isAnswer = bi === q.answer;
        const isChosenWrong = bi === oi && oi !== q.answer;
        if (!isAnswer && !isChosenWrong) return;
        // WCAG 1.4.1 (Use of Color): pair the color state with a shape glyph and
        // screen-reader-only text so correctness is also conveyed non-visually.
        b.classList.add(isAnswer ? "correct" : "wrong");
        const glyph = el("span", { class: "q-mark", "aria-hidden": "true" }, isAnswer ? "\u2713" : "\u2717");
        const srText = el("span", { class: "visually-hidden" }, isAnswer ? "Correct answer. " : "Incorrect answer. ");
        b.insertBefore(glyph, b.firstChild);
        b.insertBefore(srText, b.firstChild);
      });
