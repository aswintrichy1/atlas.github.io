...
(function () {
  ...
  /* ---------------- data model ---------------- */
  const TRACKS = [window.TRACKS.hld, window.TRACKS.lld, window.TRACKS.data].filter(Boolean);
  const QUIZZES = window.QUIZZES || {};
  const ACTIVE_QUIZ_PREFIXES = ["hld-", "lld-", "data-"];
  ...
  const Practice = window.TechLeadPractice || {};
  ...
  const LEARNING_PATHS = [
    {
      id: "opening-minutes",
      title: "The Opening Twelve Minutes",
      label: "Start here",
      color: "#f59e0b",
      desc: "What this round actually scores, and how to convert a vague business ask into architecture drivers, stakeholder positions, and a stated set of assumptions. Ends able to open any prompt with a structure that survives the first three follow-ups.",
      routes: [
        "#/hld/framing/round-decoder",
        "#/hld/framing/business-drivers",
        "#/hld/framing/stakeholder-map",
        "#/hld/execution/executive-hld",
        "#/hld/framing/assumptions-risks"
      ]
    },
    {
      ...
      label: "Judgment path",
      color: "#22d3ee",
      desc: "The material that separates an opinion from a decision: reversibility, cost as a design constraint, decision records, and knowing when an abstraction has earned its keep. Ends able to defend a consequential choice on criteria rather than preference.",
      routes: [
        "#/hld/framing/assumptions-risks",
        "#/hld/execution/cost-aware-hld",
        "#/hld/execution/adr-leadership",
        "#/lld/leadership/extensibility",
    ...
    },
    {
      ...
      color: "#fb7185",
      desc: "Rollout, incident, and cutover \u2014 the three moments where technical judgment is observed rather than described. Ends able to run a risky migration, lead the incident that follows it, and make a go/no-go call you can defend afterwards.",
      routes: [
        "#/hld/operations/operating-model",
        "#/hld/operations/migration-strategy",
        "#/hld/operations/incident-hld",
        "#/data/trust/data-sla-incident",
        "#/data/governance/migration-cutover"
      ]
    },
    {
      id: "code-credibility",
      title: "Code-Level Credibility",
      label: "LLD path",
      color: "#a78bfa",
      desc: "For the moment the interview drops from architecture to implementation and stays there. Invariants, contracts, retry safety, transaction boundaries, tests, refactoring, and review \u2014 the concrete answers that stop a leadership story from sounding second-hand.",
      routes: [
        "#/lld/modeling/hld-to-lld",
        "#/lld/modeling/domain-modeling",
        "#/lld/modeling/api-contracts",
        "#/lld/modeling/idempotency",
        "#/lld/quality/transactions",
        "#/lld/quality/testing-design",
        "#/lld/quality/safe-refactoring",
        "#/lld/leadership/code-review",
        "#/lld/leadership/quality-gates"
      ]
    },
    {
      id: "data-trust",
      title: "Owning Data Trust",
      label: "Data path",
      color: "#34d399",
      desc: "Leading a platform where the numbers are believed: metric ownership, lineage, quality controls, the real-time question, cost, governance, and how to communicate confidence during an incident. Ends able to run a reconciliation instead of joining the argument.",
      routes: [
        "#/data/trust/metric-ownership",
        "#/data/trust/lineage-impact",
        "#/data/trust/quality-operating-model",
        "#/data/platform/batch-streaming",
        "#/data/platform/platform-cost",
        "#/data/platform/governance-speed",
        "#/data/governance/data-communication"
      ]
    }
  ];

  /* ---------------- progress (localStorage) ---------------- */
  const PKEY = "tm_progress_v1";
  let done = new Set();
  let activeQuiz = null;
  ...
  function practiceHomeHtml() {
    ...
    return '<section class="practice-ref-home reveal in">' +
      '<div class="prh-head"><h2>Practice library</h2><p>Scenario packs you can talk through, timed prompts with the signals a panel listens for, the scoring rubric, cheat sheets, and a glossary of the terms you are expected to use precisely.</p></div>' +
  ...
  function renderHome() {
    document.title = "TechLead · The Techno Managerial Atlas";
    const qTotal = activeQuizKeys().reduce((a, k) => a + QUIZZES[k].questions.length, 0);
    const feat = [
      ["bolt", "Interactive labs", "Weigh a stakeholder trade-off, assemble a decision record, size the blast radius of a cutover, triage an incident to a severity, prove a retry is safe, and build a reconciliation bridge."],
    ...
    main.innerHTML =
      '<section class="hero">' +
        '<span class="hero-tag reveal reveal-1"><span class="pulse"></span>Interactive techno managerial atlas</span>' +
        '<h1 class="reveal reveal-2">Defend the <span class="grad">decision</span>,<br>not just the<br><span class="grad">diagram</span>.</h1>' +
        '<p class="lede reveal reveal-3">The round that sits between design and management: architecture judgment under real constraints, <strong>delivery risk</strong> you have to own, <strong>stakeholders</strong> who disagree for good reasons, and incidents where the first move matters more than the root cause. Read the reasoning, then <em>drive</em> it.</p>' +
        '<div class="hero-cta reveal reveal-4">' +
          (resumeF
            ? '<a class="btn btn-primary" href="' + resumeF.route + '">Resume \u00b7 ' + escapeHtml(resumeF.lesson.title) + ARR + "</a>" +
              '<a class="btn btn-ghost" href="#/hld/framing/round-decoder">Start at the beginning' + ARR + "</a>"
            : '<a class="btn btn-primary" href="#/hld/framing/round-decoder">Start at the beginning' + ARR + "</a>" +
              '<a class="btn btn-ghost" href="#/data/trust/metric-ownership">Jump to data trust' + ARR + "</a>") +
      ...
      '<a class="path-banner" href="#/paths">' +
        ...
        '<div class="pb-text"><h3>Follow a guided learning path</h3><p>Pick The Opening Twelve Minutes, Decision Discipline, Delivery Under Pressure, Code-Level Credibility, or Owning Data Trust \u2014 and copy any path as Markdown study notes.</p></div>' +
      ...
      '<a class="practice-banner" href="#/practice">' +
        ...
        '<div class="pb-text"><h3>Test yourself in Practice mode</h3><p>A shuffled mix of every module checkpoint across all three tracks \u2014 ' + qTotal + ' questions for spaced-repetition review.</p></div>' +
      ...
      '<p class="home-section-sub">Leadership judgment sticks when you have made the call yourself. Every consequential trade-off here has something you can drive.</p>' +
  ...
  function renderPaths() {
    document.title = "Learning paths · TechLead";
    ...
    main.innerHTML =
      '<article class="lesson paths-page" style="--accent:var(--cyan)">' +
        ...
        '<header class="lesson-head reveal in">' +
          ...
          '<p class="summary">Opinionated routes through existing TechLead lessons. Use them when you want a focused study sequence instead of browsing the full atlas.</p>' +
        "</header>" +
  ...
  function practicePageShell(kind, body, anchor) {
    ...
    document.title = meta.title + " · TechLead";
  ...
  function renderPractice(opts) {
    ...
    document.title = "Practice mode · TechLead";
    ...
    if (mode === "weak" && !picked.length) {
      ...
      mountQuiz($("#practiceSlot"), {
        ...
        sub: count + (mode === "weak" ? " missed question" + (count === 1 ? "" : "s") + " to re-master" : " questions, shuffled across HLD, LLD & Data"),
        questions: picked
      });
    }
    main.scrollTop = 0;
  ...
  function renderReview() {
    document.title = "Study list · TechLead";
  ...
  function renderLesson(ctx) {
    ...
    document.title = lesson.title + " · TechLead";
    saveLast(ctx.key);
  ...
  function pathToMd(path) {
    const lessons = pathLessons(path);
    let out = "# " + path.title + " \u2014 TechLead learning path\n\n";
    out += "> " + path.desc + "\n\n";
    lessons.forEach((f, i) => {
  ...
  function renderExam() {
    document.title = "Exam mode \u00b7 TechLead";
  ...
  function renderFlashcards() {
    document.title = "Flashcards \u00b7 TechLead";
  ...
  function renderPrint(trackId) {
    ...
    document.title = track.name + " cheat sheet \u00b7 TechLead";
  ...
  function exportData() {
    ...
    const payload = { app: "techlead", version: 1, exportedAt: new Date().toISOString(), data };
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = el("a", { href: url, download: "techlead-progress-" + new Date().toISOString().slice(0, 10) + ".json" });
  ...
  function importData(file) {
    ...
    reader.onload = () => {
      ...
      if (!parsed || parsed.app !== "techlead" || !parsed.data) { toast("Import failed \u2014 not a TechLead backup"); return; }
  ...
  /* ---------------- rescue: content stranded by the engine migration ----------
   * This app used to run on its own engine, which stored three things the user
   * typed by hand: a story bank, a mock-interview draft, and the history of past
   * mock runs. The shared engine has no screen that reads those keys, so the
   * data is still in localStorage and no longer reachable from anywhere in the
   * UI. Progress keys were reset by the same migration, but that is fine and
   * expected — route ids and question ids all changed, so old progress would
   * point at lessons that no longer exist. Silently stranding prose someone
   * wrote is a different matter, hence this one-time offer to get it out.
   *
   * Deliberately cheap when there is nothing to rescue: if none of the three
   * keys holds non-empty data, nothing is built and nothing is inserted.
   */
  const LEGACY_KEYS = [
    ["tm_story_bank_v1", "Story bank"],
    ["tm_interview_draft_v1", "Mock interview draft"],
    ["tm_interview_history_v1", "Mock interview history"],
  ];
  const RESCUE_SEEN = "tm_legacy_rescued_v1";

  function legacyFound() {
    const found = [];
    for (const [key, label] of LEGACY_KEYS) {
      let raw = null;
      try { raw = localStorage.getItem(key); } catch (e) { return []; }
      if (raw == null) continue;
      const text = String(raw).trim();
      /* "[]", "{}" and "" are the shapes the old engine left behind when a
         feature was opened but never used, and are not worth a notice. */
      if (!text || text === "[]" || text === "{}" || text === "null") continue;
      found.push({ key, label, raw });
    }
    return found;
  }

  function legacyAsMarkdown(found) {
    const lines = ["# TechLead \u2014 recovered notes", "", "Saved by an earlier version of this app.", ""];
    for (const item of found) {
      lines.push("## " + item.label, "");
      let parsed;
      try { parsed = JSON.parse(item.raw); } catch (e) { parsed = null; }
      if (parsed && typeof parsed === "object") {
        /* Field names came from an engine that no longer exists, so nothing here
           assumes a shape: print whatever scalar leaves are present, in order. */
        const walk = (node, depth) => {
          const pad = "  ".repeat(depth);
          if (Array.isArray(node)) {
            node.forEach((v, i) => {
              if (v && typeof v === "object") { lines.push(pad + "- item " + (i + 1)); walk(v, depth + 1); }
              else lines.push(pad + "- " + String(v));
            });
          } else if (node && typeof node === "object") {
            for (const k in node) {
              const v = node[k];
              if (v && typeof v === "object") { lines.push(pad + "- **" + k + "**"); walk(v, depth + 1); }
              else if (v != null && String(v) !== "") lines.push(pad + "- **" + k + ":** " + String(v));
            }
          } else lines.push(pad + String(node));
        };
        walk(parsed, 0);
      } else {
        lines.push(item.raw);
  ...
  function mountLegacyRescue() {
    let seen = null;
    try { seen = localStorage.getItem(RESCUE_SEEN); } catch (e) { return; }
    if (seen) return;

    const found = legacyFound();
    ...
    const settle = (msg) => {
      try { localStorage.setItem(RESCUE_SEEN, stamp); } catch (e) {}
      notice.remove();
      if (msg) toast(msg);
    };

    const notice = el("div", { class: "note key", role: "region", "aria-label": "Recovered notes from an earlier version" },
      el("svg", { class: "note-ico", viewBox: "0 0 24 24", html: '<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><path d="M7 10l5 5 5-5"/><path d="M12 15V3"/>' }),
      el("div", { class: "note-body" },
        el("p", {},
          el("strong", {}, "Notes from an earlier version of this app. "),
          "This page was rebuilt on a shared engine that has no screen for " +
          found.map((f) => f.label.toLowerCase()).join(", ") +
          ". The text is still saved in this browser \u2014 download it before it gets cleared."),
        el("div", { style: "display:flex;flex-wrap:wrap;gap:8px;margin-top:10px" },
          el("button", {
            class: "w-btn primary", type: "button",
            onclick: () => {
              const data = {};
              found.forEach((f) => { data[f.key] = f.raw; });
              saveFile(JSON.stringify({ app: "techlead", version: 1, recoveredAt: new Date().toISOString(), data }, null, 2),
                "application/json", "techlead-recovered-notes-" + stamp + ".json");
              settle("Notes downloaded as JSON");
            },
          }, "Download JSON"),
          el("button", {
            class: "w-btn", type: "button",
            onclick: () => {
              saveFile(legacyAsMarkdown(found), "text/markdown", "techlead-recovered-notes-" + stamp + ".md");
              settle("Notes downloaded as Markdown");
            },
          }, "Download Markdown"),
          el("button", { class: "w-btn ghost", type: "button", onclick: () => settle("") }, "Dismiss"))));
  ...
  /* ---------------- boot ---------------- */
  /* Every route replaces main's contents, so the rescue notice is re-offered
     after each render until it is acted on; mountLegacyRescue returns
     immediately once the flag is set, or if there was never anything to
     rescue. */
  window.addEventListener("hashchange", () => { route(); mountLegacyRescue(); });
  buildNav();
  updateProgressRing();
  route();
  mountLegacyRescue();
