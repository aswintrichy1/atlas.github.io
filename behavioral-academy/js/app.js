...
(function () {
  ...
  /* ---------------- data model ---------------- */
  const TRACKS = [window.TRACKS.beh, window.TRACKS.story, window.TRACKS.loops, window.TRACKS.offer].filter(Boolean);
  const QUIZZES = window.QUIZZES || {};
  const ACTIVE_QUIZ_PREFIXES = ["beh-", "story-", "loops-", "offer-"];
  ...
  const Practice = window.CompassPractice || {};
  ...
  const LEARNING_PATHS = [
    {
      ...
      label: "Foundations",
      color: "#fb7185",
      desc: "Why the round exists, the eight signal areas behind it, and how to name the signal and pick the story before you say a word.",
      routes: [
        "#/beh/foundation/why-it-matters",
        "#/beh/foundation/how-evaluated",
        "#/beh/foundation/decode",
        "#/beh/foundation/select"
      ]
    },
    {
      id: "beh-delivery",
      ...
      label: "Delivery",
      color: "#fbbf24",
      desc: "SALT, the three questions you will always be asked, and the pitfalls that quietly cost the offer.",
      routes: [
        "#/beh/delivery/deliver-salt",
        "#/beh/delivery/big-three",
        "#/beh/delivery/pitfalls"
      ]
    },
    {
      id: "beh-advanced",
      ...
      label: "Advanced",
      color: "#38bdf8",
      desc: "Special interview types, the new questions about working with AI, and how to actually practise.",
      routes: [
        "#/beh/advanced/special-types",
        "#/beh/advanced/ai-questions",
        "#/beh/advanced/practicing"
      ]
    },
    {
      id: "story-bank",
      ...
      label: "Stories",
      color: "#34d399",
      desc: "Engineer a catalog bottom-up, prove scope, and map every story onto the signals interviewers score.",
      routes: [
        "#/story/catalog/journaling",
        "#/story/catalog/story-anatomy",
        "#/story/catalog/scope-signal",
        "#/story/catalog/ownership-ambiguity",
        "#/story/catalog/conflict-growth",
        "#/story/catalog/coverage-matrix"
      ]
    },
    {
      id: "story-playbooks",
      ...
      label: "Playbooks",
      color: "#a78bfa",
      desc: "Adapt the same catalog to big-tech loops, principle-based rubrics, startups, and the manager track.",
      routes: [
        "#/story/playbooks/big-tech",
        "#/story/playbooks/principle-based",
        "#/story/playbooks/startup-vs-scale",
        "#/story/playbooks/manager-track",
        "#/story/playbooks/mock-drills"
      ]
    },
    {
      id: "company-loops",
      title: "Senior & Above Playbooks",
      label: "Target your loop",
      color: "#818cf8",
      desc: "The senior, staff and manager loops, round by round, with the rubric each is scored against and what candidates consistently find surprising. Ends knowing which lesson prepares each round.",
      routes: [
        "#/loops/meta/meta-e5",
        "#/loops/meta/meta-e6",
        "#/loops/meta/meta-m1",
        "#/loops/amazon/amazon-l5",
        "#/loops/amazon/amazon-l6",
        "#/loops/amazon/amazon-manager",
        "#/loops/google/google-l5",
        "#/loops/google/google-l6",
        "#/loops/google/google-manager",
        "#/loops/others/openai-l5",
        "#/loops/others/airbnb-g9",
        "#/loops/others/microsoft-senior"
      ]
    },
    {
      id: "early-loops",
      title: "Early-Career Playbooks",
      label: "First loops",
      color: "#4ade80",
      desc: "The entry and mid-level loops, where the bar is competence and clear communication rather than scope. Ends knowing what is actually being measured before seniority enters the picture.",
      routes: [
        "#/loops/meta/meta-e3",
        "#/loops/meta/meta-e4",
        "#/loops/amazon/amazon-l4",
        "#/loops/google/google-l4",
        "#/loops/others/openai-l4",
        "#/loops/others/airbnb-g8"
      ]
    },
    {
      id: "offer-close",
      ...
      color: "#fbbf24",
      desc: "The last conversation, treated as seriously as the interviews. Ends knowing which parts of an offer actually move, why level negotiation dominates comp negotiation, and what to say.",
      routes: [
        "#/offer/anatomy/components",
        "#/offer/anatomy/level-bands",
        "#/offer/anatomy/leverage",
        "#/offer/execution/recruiter-scope",
        "#/offer/execution/counter-scripts",
        "#/offer/execution/mistakes",
        "#/offer/close/accept-decline"
      ]
    }
  ];

  /* ---------------- progress (localStorage) ---------------- */
  const PKEY = "cp_progress_v1";
  let done = new Set();
  let activeQuiz = null;
  ...
  function renderHome() {
    document.title = "Compass · The Behavioral Interview Atlas";
    const qTotal = activeQuizKeys().reduce((a, k) => a + QUIZZES[k].questions.length, 0);
    const feat = [
      ["bolt", "Interactive labs", "Grade a story against the signal rubric, drill the story shortlist on a live prompt, run a SALT timer, map your catalog onto a coverage matrix, and spot the missing “I” in a transcript."],
    ...
    main.innerHTML =
      '<section class="hero">' +
        '<span class="hero-tag reveal reveal-1"><span class="pulse"></span>Interactive behavioral-interview atlas</span>' +
        '<h1 class="reveal reveal-2">Tell stories<br>that <span class="grad">land</span> &amp; show<br>scope that <span class="grad">scales</span>.</h1>' +
        '<p class="lede reveal reveal-3">Treat the behavioral round like the engineering problem it is \u2014 name the signal behind the question, pick the story that proves it, and land it so the interviewer can score you. Then build a reusable story bank that covers every signal area you will be asked about.</p>' +
        '<div class="hero-cta reveal reveal-4">' +
          (resumeF
            ? '<a class="btn btn-primary" href="' + resumeF.route + '">Resume \u00b7 ' + escapeHtml(resumeF.lesson.title) + ARR + "</a>" +
              '<a class="btn btn-ghost" href="#/beh/foundation/why-it-matters">Start the course' + ARR + "</a>"
            : '<a class="btn btn-primary" href="#/beh/foundation/why-it-matters">Start the course' + ARR + "</a>" +
              '<a class="btn btn-ghost" href="#/story/catalog/journaling">Build a story bank' + ARR + "</a>") +
      ...
      '<a class="practice-banner" href="#/practice">' +
        ...
        '<div class="pb-text"><h3>Test yourself in Practice mode</h3><p>A shuffled mix of every COURSE and STORIES checkpoint quiz \u2014 ' + qTotal + ' questions for spaced-repetition review.</p></div>' +
  ...
  function renderPaths() {
    document.title = "Learning paths · Compass";
    ...
    main.innerHTML =
      '<article class="lesson paths-page" style="--accent:var(--cyan)">' +
        ...
        '<header class="lesson-head reveal in">' +
          ...
          '<p class="summary">Opinionated routes through existing Compass lessons. Use them when you want a focused study sequence instead of browsing the full atlas.</p>' +
        "</header>" +
  ...
  function practicePageShell(kind, body, anchor) {
    ...
    document.title = meta.title + " · Compass";
  ...
  function renderPractice(opts) {
    ...
    document.title = "Practice mode · Compass";
    ...
    if (mode === "weak" && !picked.length) {
      ...
      mountQuiz($("#practiceSlot"), {
        ...
        sub: count + (mode === "weak" ? " missed question" + (count === 1 ? "" : "s") + " to re-master" : " questions, shuffled across COURSE & STORIES"),
        questions: picked
      });
    }
    main.scrollTop = 0;
  ...
  function renderReview() {
    document.title = "Study list · Compass";
  ...
  function renderLesson(ctx) {
    ...
    document.title = lesson.title + " · Compass";
    saveLast(ctx.key);
  ...
  function pathToMd(path) {
    const lessons = pathLessons(path);
    let out = "# " + path.title + " \u2014 Compass learning path\n\n";
    out += "> " + path.desc + "\n\n";
    lessons.forEach((f, i) => {
  ...
  function renderExam() {
    document.title = "Exam mode \u00b7 Compass";
  ...
  function renderFlashcards() {
    document.title = "Flashcards \u00b7 Compass";
  ...
  function renderPrint(trackId) {
    ...
    document.title = track.name + " cheat sheet \u00b7 Compass";
  ...
  function exportData() {
    ...
    const payload = { app: "compass", version: 1, exportedAt: new Date().toISOString(), data };
    const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = el("a", { href: url, download: "compass-progress-" + new Date().toISOString().slice(0, 10) + ".json" });
  ...
  function importData(file) {
    ...
    reader.onload = () => {
      ...
      if (!parsed || parsed.app !== "compass" || !parsed.data) { toast("Import failed \u2014 not a Compass backup"); return; }
